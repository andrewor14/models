...
if [[ "$MODE" == "autoscaling" ]] || [[ "$MODE" == "checkpoint-restart" ]]; then
  ...
  if [[ "$DATASET" == "cifar10" ]]; then
    ...
    export AUTOSCALING_MAX_LOCAL_BATCH_SIZE="${AUTOSCALING_MAX_LOCAL_BATCH_SIZE:=256}"
    export AUTOSCALING_MAX_GLOBAL_BATCH_SIZE="${AUTOSCALING_MAX_GLOBAL_BATCH_SIZE:=1024}"
  elif [[ "$DATASET" == "imagenet" ]]; then
    export AUTOSCALING_MAX_LOCAL_BATCH_SIZE="${AUTOSCALING_MAX_LOCAL_BATCH_SIZE:=128}"
    export AUTOSCALING_MAX_GLOBAL_BATCH_SIZE="${AUTOSCALING_MAX_GLOBAL_BATCH_SIZE:=8192}"
  fi
fi
